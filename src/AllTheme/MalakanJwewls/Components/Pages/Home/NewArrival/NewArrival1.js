...
const NewArrival = () => {
    ...
    return (
        <div className='mala_newwArr1MainDiv'>
            <Typography variant='h4' className='mala_NewArr1Title'>NEW ARRIVAL
                <Link className='mala_designSetViewmoreBtn' onClick={() => navigation(`/p/NewArrival/?N=${btoa('NewArrival')}`)}>
                    View more
                </Link>
            </Typography>
            {newArrivalData &&
                <Grid container spacing={1} className='mala_NewArrival1product-list'>
                    {newArrivalData?.slice(0, 4)?.map((product, index) => (
                        <Grid item xs={6} sm={4} md={3} lg={3} key={index}>
                            <Card className='mala_NewArrproduct-card' onClick={() => handleNavigation(product?.designno, product?.autocode, product?.TitleLine)}>
                                <div className='mala_newArr1Image'>
                                    <CardMedia
                                        component="img"
                                        className='mala_newArrImage'
                                    ...
                                    <Typography variant='body2'>
                                        {storeInit?.IsGrossWeight == 1 &&
                                            <>
                                                ...
                                                <span className='mala_lb3detailDT'>{(product?.Gwt || 0)?.toFixed(3)}</span>
                                            </>
                                        }
                                        {storeInit?.IsGrossWeight == 1 &&
                                            <>
                                                ...
                                                <span className='mala_lb3detailDT'>{(product?.Nwt || 0)?.toFixed(3)}</span>
                                            </>
                                        }
                                        {storeInit?.IsGrossWeight == 1 &&
                                            <>
                                                {(product?.Dwt != "0" || product?.Dpcs != "0") &&
                                                    <>
                                                        ...
                                                        <span className='mala_lb3detailDT'>{(product?.Dwt || 0)?.toFixed(3)} / {(product?.Dpcs || 0)}</span>
                                                    </>
                                                }
                                            </>
                                        }
                                            ...
                                            <>
                                                {(product?.CSwt != "0" || product?.CSpcs != "0") &&
                                                    <>
                                                        ...
                                                        <span className='mala_lb3detailDT'>{(product?.CSwt || 0)?.toFixed(3)} / {(product?.CSpcs || 0)}</span>
                                                    </>
                                                }
                                            </>
                                        }
                                    </Typography>
                                    <p className='mala_newArrPrice'>
                                        <span
