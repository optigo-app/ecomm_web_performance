...
const CustomSortIcon = ({ order }) => {
  return (
    <>
      {order === 'asc' && <ArrowUpwardIcon className='sorticon_ma_DT' />}
      {order === 'desc' && <ArrowDownwardIcon className='sorticon_ma_DT' />}
    </>
  );
};
...
const scrollToTop = () => {
  ...
  return (
    <div className='quotationJob_Account_DT'>
                    ...
                    <TableCell style={{backgroundColor: "#f0e0e0", color: "black"}}>
                      ...
                      {columns?.slice(1)?.map((column) => (
                        ...
                        >
                          {column.label}
                          {orderBy === column.id ? (
                            <span style={{ display: 'flex', alignItems: 'right' }} className='sorticon_ma_span_DT'>
